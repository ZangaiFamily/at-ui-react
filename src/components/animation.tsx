...
function startAnimationHelper(callback: Function) {
...
function isPercentage(height: string | number) {
    // Percentage height
...
function runCallback(callback: Function, params: any) {
...
export class AtAnimationHeight extends React.Component<IAnimationHeight, any> {
    ...
    componentDidUpdate(prevProps: IAnimationHeight, prevState: IAnimationHeight) {
        const {
            delay,
            duration,
            height,
        ...
        if (this.contentElement && height !== prevProps.height) {
            ...
            if (isCurrentHeightAuto) {
                ...
                startAnimationHelper(() => {
                    ...
                    if (onAnimationStart) runCallback(onAnimationStart, {newHeight: timeoutState.height});
                });
                ...
                this.animationClassesTimeoutID = setTimeout(() => {
                    ...
                    if (onAnimationEnd) runCallback(onAnimationEnd, {newHeight: timeoutState.height});
                }, totalDuration);
            } else {
                // ANIMATION STARTS, run a callback if it exists
                if (onAnimationStart) runCallback(onAnimationStart, {newHeight});
                ...
                this.timeoutID = setTimeout(() => {
                    ...
                    // Run a callback if it exists
                    if (onAnimationEnd) runCallback(onAnimationEnd, {newHeight});
                }, totalDuration);
            }
        }
    }
    ...
    showContent(height: number | string) {
        if (height === 0) {
            this.contentElement.style.display = '';
        }
    }

    hideContent(newHeight: number) {
        if (newHeight === 0) {
            this.contentElement.style.display = 'none';
        }
    }

    getStaticStateClasses(height: number | string) {
        return cx({
