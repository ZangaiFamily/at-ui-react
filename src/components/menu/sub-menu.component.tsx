...
export class AtSubMenuComponent extends React.Component<ISubMenu, any> {
    ...
    public render() {
        ...
        const position = this.props.level === 0 ? 'bottom' : 'right';

        return (<li className={classes}>
                <div onClick={this.handleClick} onMouseEnter={this.handleEnter} onMouseLeave={this.handleLeave}
                     ref={this.setTitle} className={'at-menu__submenu-title'}>
                    {this.props.title} {this.props.level}
                </div>
                <Transition in={this.state.open} timeout={15}>
                    {(state: any) => (
                        <ConnectedOverlay
                            position={position}
